...
if ($whmcs->get_req_var("infopopup")) {
	$data = curlCall("http://api.mtimer.net/whmcs/popup/popup.php", array("licensekey" => $whmcs->get_license_key(), "version" => $whmcs->get_config("Version"), "ssl" => (((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']) && $_SERVER['HTTPS'] != "off") ? "1" : "0")));
...
if ($whmcs->get_req_var("toggleinfopopup")) {
	...
	if ($showhide == "true") {
		$infotoggle[$_SESSION['adminid']] = curlCall("http://api.mtimer.net/whmcs/popup/popup.php", "lastupdate=1", array("CURLOPT_TIMEOUT" => "5"));
	}
	else {
		if ($showhide == "false") {
...
if (get_query_val("tbladmins", "roleid", array("id" => (int)$_SESSION['adminid'])) == 1) {
	...
	if ($infotoggle[$_SESSION['adminid']]) {
		...
		$lastupdate = curlCall("http://api.mtimer.net/whmcs/popup/popup.php", "lastupdate=1", array("CURLOPT_TIMEOUT" => "5"));
